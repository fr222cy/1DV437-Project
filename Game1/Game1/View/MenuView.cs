...
namespace Game1.View
{
    class MenuView
    {
        ...
        public int Update(Vector2 mousePosition , Texture2D playButton, bool hasClicked, GraphicsDeviceManager graphics)
        {
            ...
            //    }
            //}
            if (pos_exit.X >= mouseModelPosition.X - buttonModelWidth && pos_exit.X <= mouseModelPosition.X + buttonModelWidth
            && pos_exit.Y >= mouseModelPosition.Y - buttonModelHeight && pos_exit.Y <= mouseModelPosition.Y + buttonModelHeight)
            {
                if (hasClicked)
                {
                    return 2;     
